...
function openRoomModal(inputElement) {
    activeInput = inputElement;
    const modal = document.getElementById('roomModal');
    const searchInput = document.getElementById('modalSearch');
    const roomsList = document.getElementById('modalRoomsList');
    
    // Собираем все аудитории из coordinates
    allRoomsList = Object.keys(graphData.coordinates).filter(k => /^\d{4}$/.test(k));
    
    searchInput.value = '';
    renderRoomsList(allRoomsList);
...
function renderRoomsList(rooms) {
    ...
    if (rooms.length === 0) {
        roomsList.innerHTML = '<div style="padding: 20px; text-align: center; color: #888;">Ничего не найдено</div>';
        return;
    }
    ...
    rooms.forEach(room => {
        ...
        item.onclick = () => {
            if (activeInput) activeInput.value = room;
            closeModal();
        };
...
document.getElementById('modalSearch').addEventListener('input', (e) => {
    ...
    const filtered = allRoomsList.filter(room => room.includes(query));
...
// Назначаем открытие модалки на поля ввода (после загрузки данных)
function setupModalTriggers() {
    const fromInput = document.getElementById('from');
    const toInput = document.getElementById('to');
    
    fromInput.addEventListener('click', () => openRoomModal(fromInput));
    toInput.addEventListener('click', () => openRoomModal(toInput));
}

// Вызываем после загрузки данных
const originalLoadGraphData = loadGraphData;
window.loadGraphData = async function() {
    await originalLoadGraphData();
    setupModalTriggers();
};
