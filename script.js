...
if (finishBtn) finishBtn.addEventListener('click', finishRoute);

// Перехватываем showStep, чтобы обновлять состояние кнопок
const originalShowStep = showStep;
window.showStep = function() {
    originalShowStep();
    updateRouteControls();
};
showStep = window.showStep;

// И в startNavigation тоже добавляем сброс управления
const originalStartNavigation = startNavigation;
window.startNavigation = function(start, end) {
    originalStartNavigation(start, end);
    updateRouteControls();
};
startNavigation = window.startNavigation;
